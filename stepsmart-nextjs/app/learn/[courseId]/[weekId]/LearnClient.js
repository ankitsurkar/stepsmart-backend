...
export default function LearnClient({
  ...
  // Sync state with server-side fetched data on mount and page transition
  useEffect(() => {
    const isRecordedSession = String(weekId).startsWith('rec-');

    const modulesList = initialWeeks.modules || initialWeeks.weeks || [];
    const liveWeeksList = initialWeeks.liveWeeks || [];
    const supplementalList = initialWeeks.supplementalContent?.liveRecordedSessions || [];
    ...
    if (!found) {
      setError('Content not found or not yet released.');
      setLoading(false);
      return;
    }
    ...
    const weekProgress = initialProgressMap[weekId] || null;
    setProgress(weekProgress);
    
    const progressList = Object.values(initialProgressMap);
    ...
    setQuestionsList(initialQAQuestions);
    setLoading(false);
  }, [courseId, weekId, initialWeeks, initialProgressMap, initialQAQuestions, activeTab]);

  async function loadWeek() {
    // Client-side refresh (revalidation)
    try {
      const [progressRes, qaRes] = await Promise.all([
        getProgress(courseId),
        getQAQuestions(courseId, weekId).catch(() => ({ data: { questions: [] } })),
      ]);

      const freshProgress = progressRes.data.progress || [];
      const weekProgress = freshProgress.find((p) => p.weekId === weekId) || null;
      
      setProgress(weekProgress);
      setAllProgress(freshProgress);
      ...
      console.error('Failed to reload progress data client-side:', err);
    }
  }
        ...
        <div style={{ flex: 1, minWidth: 0, height: isCompact ? 'auto' : '100vh', overflowY: isCompact ? 'visible' : 'auto' }}>
          ...
          {isCompact && (
            <div style={{
              ...
              <Link href="/dashboard?view=courses" style={{ ...s.backLink, color: 'var(--primary)', fontWeight: 700 }}>
                ← Back to course
              </Link>
              <button
              ...
              {(() => {
                ...
                return (
                  <div style={s.nextBtnContainer}>
                    <Link href={`/learn/${courseId}/${nextWeek.weekId || nextWeek.id}`} style={s.nextBtn}>
                      Next lesson →
                    </Link>
                  </div>
                );
              ...
              {(() => {
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      setShowCompletionModal(false);
                       router.push(`/learn/${courseId}/${nextWeek.weekId || nextWeek.id}`);
                    }}
                  >
                    Next lesson
                  </button>
